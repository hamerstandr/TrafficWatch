...
namespace EarTrumpet.Services
{
    public class ThemeService
    {
        ...
        private static Color GetWindowBackgroundColor()
        {
            ...
            var color = AccentColorService.GetColorByTypeName(resource);//255,31,31,31
        ...
        {
            var color = AccentColorService.GetColorByTypeName(immersiveAccentName);//wait
            color.A = (byte) (opacity*255);
            try
            {
                ((SolidColorBrush)dictionary[name]).Color = color;
            }
            catch { dictionary[name] = new SolidColorBrush(color); }
